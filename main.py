...
def download_videos(title,url):
    ...
    i = requests.get(url+'640x360/video.m3u8',
                     headers=headers)
    ts = i.text
...
BASE_URL='https://sisterasian.com/';
html = requests.get(BASE_URL)
html=etree.HTML(html.text)
title=html.xpath('//div[@class="video-title"]/a//text()')
url=html.xpath('//div[@class="video-card-image"]/center//@data-src')
title = [str(p) for p in title]
url = [str(p) for p in url]

dict = {title[i]:url[i] for i in range(len(title))}

for key,value in dict.items():
    value=value[0:value.rfind('/', 1) + 1]
    print(key,"\n",value)
    download_videos(key,value)